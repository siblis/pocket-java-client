...
public class AuthController {
    ...
    private AuthFromServer sendRegRequest(String name, String password, String email) {
        ...
        try {
            int responseCode = HTTPSRequest.sendRequest("/auth/registration", "POST", jsonRegData, null);
            switch (responseCode) {
                case 201:
                    return HTTPSRequest.getResponse(AuthFromServer.class);
                case 429:
    ...
    private AuthFromServer sendAuthRequest(String login, String password) {
        if (!login.isEmpty() && !password.isEmpty()) {
            ...
            try {
                int responseCode = HTTPSRequest.sendRequest("/auth/login", "POST", jsonLoginData, null);
                switch (responseCode) {
                    case 200:
                        return HTTPSRequest.getResponse(AuthFromServer.class);
                    case 429:
